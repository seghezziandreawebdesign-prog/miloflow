...
- **Eliminazione**. Le task si eliminano davvero, insieme alle sottotask. I progetti invece si archiviano soltanto. Il cestino è sulla riga (su desktop compare passando col mouse) e chiede conferma.
- **Selezione multipla** (`components/selezione.tsx`). «Seleziona» nelle viste a lista (non nel kanban né in Pianifica settimana) mostra le caselle; Shift+clic seleziona un intervallo. La barra in basso: completa (le ricorrenti creano la prossima occorrenza), data di inizio, priorità, stato, progetto, elimina. Le Server Actions `updateTasks`, `completaTasks` e `deleteTasks` accettano al massimo 200 id. Lo stesso nei Servizi: disdici, archivia, riattiva, elimina (`setStatoServizi`, `deleteServizi`).
- **Pannello della task**: quasi a tutto schermo, su desktop a due colonne con la descrizione a tutta altezza.
...
- **Pagina `/budget`** (voce «Budget & Spese») con cinque sezioni sul parametro `?tab=`: *Mese* (`?mese=yyyy-MM-01`), *Debiti*, *Risparmi*, *Investimenti* e *Report* (`?periodo=mese|3mesi|12mesi|anno`, oppure `?periodo=personalizzato&dal=yyyy-MM-dd&al=yyyy-MM-dd`; date non valide o invertite tornano a «Questo mese»). Il mese mostra budget, speso, previsto e rimanente, una barra per categoria (le sottocategorie contano nel padre) e i movimenti raggruppati per giorno. Un movimento appartiene al mese per la sua `data`; `periodo` è il mese di competenza usato dai vincoli univoci.
...
- **Risparmi e investimenti** (tabella `salvadanai`, `tipo` = `risparmio` | `investimento`). Un risparmio ha un obiettivo e una data facoltativa (l'app calcola quanto versare al mese); un investimento ha fondo, ISIN e piattaforma. I **versamenti sono movimenti** del budget con `salvadanaio_id`: contano come spesa del mese nella categoria del salvadanaio (di default *Risparmi e investimenti › Risparmi/Investimenti*, create dalla migration). I **prelievi** (`salvadanai_prelievi`) e le **valutazioni** degli investimenti (`salvadanai_valori`, una per giorno) non toccano il budget. La vista `v_salvadanai` dà versato, prelevato, saldo, prossimo previsto e ultimo valore. Con `importo_mensile` e `giorno_mensile` c'è un **piano**: `genera_previsti` e i trigger creano un previsto al mese (vincolo `(salvadanaio_id, periodo)`), mai con una data precedente alla creazione; «Segna versato» lo rende pagato. I versamenti a mano hanno `periodo` null. Sospendere o archiviare toglie i previsti non pagati; si elimina solo senza versamenti né prelievi (`elimina_salvadanaio`), altrimenti si archivia.
...
In Impostazioni → Backup (solo owner). **Scarica il backup** produce uno ZIP costruito nel browser con `fflate`: `dati.json` (tutte le tabelle, da `esporta_backup()`, compresi risparmi, investimenti, prelievi e valutazioni), `LEGGIMI.txt` e la cartella `file/<bucket>/<percorso>` con loghi, ricevute e allegati delle task scaricati dallo Storage. Le credenziali cifrate restano cifrate: servono la stessa master password e nient'altro.
